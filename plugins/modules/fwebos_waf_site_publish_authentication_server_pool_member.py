...
DOCUMENTATION = """
---
module: fwebos_waf_site_publish_authentication_server_pool_member
short_description: Config FortiWeb Published Site Policy's Authentication Server Pool's Members
description:
...
options:
    name:
        description:
            - The name of the Published Site Policy's Authentication Server Pool.
        type: string
    id:
        description:
            - The numerical ID of Authentication Server Pool's member.
        type: string
    server_type:
        description:
...
obj_url = '/api/v2.0/cmdb/waf/site-publish-helper.authentication-server-pool/members'
...
def param_check(module, connection):
    ...
    if (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    res, err_msg = value_check(module.params, 'server_type', ['ldap', 'radius'])
    if res == False:
      return res, err_msg          
