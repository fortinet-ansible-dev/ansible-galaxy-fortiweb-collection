...
DOCUMENTATION = """
---
module: fwebos_certificate_xml_certificate_client
short_description: Config FortiWeb server objects XML Certificate Client Certificate
description:
...
options:
    ...
    keyfile:
        description:
            - keyfile
        type: string
    cgi_find_upfname(cgi:
        description:
            - cgi find upfname(cgi
        type: string
"""
...
obj_url = '/api/v2.0/system/certificate.xmlclientcertificate'
del_url = '/api/v2.0/cmdb/system/certificate.xml-client-certificate'
...
def main():
    ...
    if 'errcode' in str(result):
        result['changed'] = False
        result['failed'] = True
        if 'results' in str(result) and result['res']['results']['errcode'] == -3:  
            # process delete non-existent items
            result['failed'] = False
