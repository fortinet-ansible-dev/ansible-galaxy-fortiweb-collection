...
DOCUMENTATION = """
---
module: fwebos_waf_json_protection_policy
short_description: Config FortiWeb JSON Protection Policy
description:
...
obj_url = '/api/v2.0/cmdb/waf/json-validation.policy'


rep_dict = {
    "enable_attack_signatures":"enable-attack-signatures"
}
...
def param_check(module, connection):
    ...
    if (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    err_msg = ''
    res, err_msg = value_check(module.params, 'enable_attack_signatures', ['enable', 'disable'])
    if res == False:
        return res, err_msg
...
def main():
    argument_spec = dict(
        ...
        name=dict(type='str'),
        vdom=dict(type='str'),
        enable_attack_signatures=dict(type='str')
    )
