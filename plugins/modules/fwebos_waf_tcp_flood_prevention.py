...
DOCUMENTATION = """
---
module: fwebos_waf_tcp_flood_prevention
short_description: Config FortiWeb TCP Flood Prevention
description:
  - Config FortiWeb TCP Flood Prevention
version_added: "7.0.0"
author:
  - Joseph Chen
requirements:
...
options:
    name:
        description:
            - The name of the TCP flood prevention rule.
        type: string
    layer4_connection_threshold:
        description:
            - Layer 4 connection threshold.
        type: integer
    tcp_action:
        description:
            - Action.
...
EXAMPLES = """
    - name: add a TCP flood prevention rule
      fwebos_waf_tcp_flood_prevention:
       action: add
       name: tcp_flood_ansible_test
       layer4_connection_threshold: 100
       tcp_action: alert
       block_period: 600
       severity: High

    - name: edit a TCP flood prevention rule
      fwebos_waf_tcp_flood_prevention:
       action: edit
       name: tcp_flood_ansible_test
       layer4_connection_threshold: 200
       tcp_action: block-period
       block_period: 900
       severity: Medium

    - name: get a TCP flood prevention rule
      fwebos_waf_tcp_flood_prevention:
       action: get
       name: tcp_flood_ansible_test

    - name: delete a TCP flood prevention rule
      fwebos_waf_tcp_flood_prevention:
       action: delete
       name: tcp_flood_ansible_test
"""
...
obj_url = '/api/v2.0/cmdb/waf/layer4-connection-flood-check-rule'


rep_dict = {
    'layer4_connection_threshold': 'layer4-connection-threshold',
    'tcp_action': 'action',
...
def param_check(module, connection):
    ...
    if res and (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    if res:
        res, err_msg = value_check(module.params, 'tcp_action', ['alert', 'alert_deny', 'deny_no_log', 'block-period'])
    if res:
        res, err_msg = value_check(module.params, 'severity', ['High', 'Medium', 'Low', 'Info'])
    if res:
        res, err_msg = range_check(module.params, 'layer4_connection_threshold', 1, 65535)
    if res:
...
def main():
    argument_spec = dict(
        ...
        name=dict(type='str'),
        layer4_connection_threshold=dict(type='int'),
        tcp_action=dict(type='str'),
