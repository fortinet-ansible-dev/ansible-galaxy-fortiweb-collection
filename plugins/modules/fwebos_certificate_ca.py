...
DOCUMENTATION = """
---
module: fwebos_certificate_ca
short_description: Config FortiWeb server objects CA
description:
...
obj_url = '/api/v2.0/system/certificate.ca'
del_url = '/api/v2.0/cmdb/system/certificate.ca'
...
def add_obj_ca(module, connection):
    payload1 = {}
...
def add_obj(module, connection):
    if(module.params['type'] == 'localPC'):
        return add_obj_ca(module, connection)
...
def main():
    ...
    if not param_pass:
        ...
        if 'results' in data.keys() and data['results'] and type(data['results']) is not int:
            ...
            result['changed'] = True
            if module._diff:
                result['diff']['after']['deleted'] = "certificate is deleted."
        else:
            res = False
