...
DOCUMENTATION = """
---
module: fwebos_certificate_sni_member
short_description: Config FortiWeb server objects SNI Inline SNI member
description:
...
obj_url = '/api/v2.0/cmdb/system/certificate.sni/members'

rep_dict = {
    'domain_type': 'domain-type',
    'multi_local_cert': 'multi-local-cert',
    'certificate_type': 'certificate-type',
    'local_cert': 'local-cert',
    'inter_group': 'inter-group',
    'lets_certificate': 'lets-certificate',
    'multi_local_cert_group': 'multi-local-cert-group',
}
...
def main():
    argument_spec = dict(
        ...
        multi_local_cert=dict(type='str'),
        certificate_type=dict(type='str'),
        local_cert=dict(type='str'),
        inter_group=dict(type='str'),
        lets_certificate=dict(type='str'),
        multi_local_cert_group=dict(type='str'),
    ...
    # result['auth_json'] = connection.change_auth_for_vdom('test')
    ...
    if action == 'add':
        ...
        #if 1. edit the non-exist entry or 2. the content to be edited is the same as the orginal, the action will not change
        code, data = get_obj(module, connection)
