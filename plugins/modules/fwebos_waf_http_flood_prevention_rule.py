...
DOCUMENTATION = """
---
module: fwebos_waf_http_flood_prevention_rule
short_description: Config FortiWeb HTTP Flood Prevention Rule
description:
  - Config FortiWeb HTTP Flood Prevention Rule
version_added: "7.0.0"
author:
  - Joseph Chen
requirements:
...
options:
    name:
        description:
            - The name of the HTTP flood prevention rule.
        type: string
...
EXAMPLES = """
    - name: add an HTTP flood prevention rule
      fwebos_waf_http_flood_prevention_rule:
       action: add
       name: http_flood_ansible_test
       access_limit_in_http_session: 100
       flood_action: alert
       block_period: 600
       severity: High
    ...
    - name: edit an HTTP flood prevention rule
      fwebos_waf_http_flood_prevention_rule:
       action: edit
       name: http_flood_ansible_test
       access_limit_in_http_session: 200
       flood_action: block-period
       block_period: 900
       severity: Medium

    - name: get an HTTP flood prevention rule
      fwebos_waf_http_flood_prevention_rule:
       action: get
       name: http_flood_ansible_test

    - name: delete an HTTP flood prevention rule
      fwebos_waf_http_flood_prevention_rule:
       action: delete
       name: http_flood_ansible_test
"""
...
obj_url = '/api/v2.0/cmdb/waf/http-request-flood-prevention-rule'


rep_dict = {
    'access_limit_in_http_session': 'access-limit-in-http-session',
    'flood_action': 'action',
...
def param_check(module, connection):
    ...
    if res and (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    if res:
        res, err_msg = value_check(module.params, 'flood_action', ['alert', 'alert_deny', 'deny_no_log', 'block-period', 'client-id-block-period'])
    if res:
    ...
    if res:
        res, err_msg = range_check(module.params, 'access_limit_in_http_session', 0, 4096)
    if res:
...
def main():
    argument_spec = dict(
        ...
        name=dict(type='str'),
        access_limit_in_http_session=dict(type='int'),
        flood_action=dict(type='str'),
