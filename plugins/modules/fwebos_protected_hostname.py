...
DOCUMENTATION = """
---
module: fwebos_protected_hostname
short_description: Config FortiWeb Protected Hostname
description:
  - Config FortiWeb Protected Hostname
version_added: "7.0.0"
author:
  - Joseph Chen
requirements:
...
options:
    name:
        description:
            - The protected hostname policy name.
        type: string
    default_action:
        description:
            - Default action.
...
EXAMPLES = """
    - name: add a protected hostname
      fwebos_protected_hostname:
       action: add
       name: protected_hostname_ansible_test
       default_action: allow

    - name: get a protected hostname
      fwebos_protected_hostname:
       action: get
       name: protected_hostname_ansible_test

    - name: edit a protected hostname
      fwebos_protected_hostname:
       action: edit
       name: protected_hostname_ansible_test
       default_action: deny_no_log

    - name: delete a protected hostname
      fwebos_protected_hostname:
       action: delete
       name: protected_hostname_ansible_test
"""
...
obj_url = '/api/v2.0/cmdb/server-policy/allow-hosts'
...
def param_check(module, connection):
    ...
    if res and (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    if res:
        res, err_msg = value_check(module.params, 'default_action', ['allow', 'deny_no_log', 'deny'])
