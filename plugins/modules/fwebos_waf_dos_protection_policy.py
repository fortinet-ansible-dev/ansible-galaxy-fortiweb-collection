...
DOCUMENTATION = """
---
module: fwebos_waf_dos_protection_policy
short_description: Config FortiWeb DoS Protection Policy
description:
  - Config FortiWeb DoS Protection Policy
version_added: "7.0.0"
author:
  - Joseph Chen
requirements:
...
options:
    ...
    enable_http_session_based_prevention:
        description:
            - Enable HTTP session based prevention.
        type: string
        choices:
            - 'enable'
            - 'disable'
    http_request_flood_prevention_rule:
        description:
            - HTTP request flood prevention rule name.
        type: string
    http_connection_flood_check_rule:
        description:
            - HTTP connection flood check rule name.
        type: string
    enable_layer4_dos_prevention:
        description:
            - Enable Layer 4 DoS prevention.
        type: string
        choices:
            - 'enable'
            - 'disable'
    layer4_access_limit_rule:
        description:
            - Layer 4 access limit rule name.
        type: string
    layer4_connection_flood_check_rule:
        description:
            - Layer 4 connection flood check rule name.
        type: string
...
EXAMPLES = """
    - name: add a DoS protection policy
      fwebos_waf_dos_protection_policy:
       action: add
       name: dos_policy_ansible_test
       enable_http_session_based_prevention: disable
       enable_layer4_dos_prevention: disable
       layer3_fragment_protection: disable

    - name: edit a DoS protection policy
      fwebos_waf_dos_protection_policy:
       action: edit
       name: dos_policy_ansible_test
       enable_http_session_based_prevention: disable
       enable_layer4_dos_prevention: disable
       layer3_fragment_protection: enable

    - name: get a DoS protection policy
      fwebos_waf_dos_protection_policy:
       action: get
       name: dos_policy_ansible_test

    - name: delete a DoS protection policy
      fwebos_waf_dos_protection_policy:
       action: delete
       name: dos_policy_ansible_test
"""
...
obj_url = '/api/v2.0/cmdb/waf/application-layer-dos-prevention'


rep_dict = {
    'enable_http_session_based_prevention': 'enable-http-session-based-prevention',
    'http_request_flood_prevention_rule': 'http-request-flood-prevention-rule',
    'http_connection_flood_check_rule': 'http-connection-flood-check-rule',
    'enable_layer4_dos_prevention': 'enable-layer4-dos-prevention',
    'layer4_access_limit_rule': 'layer4-access-limit-rule',
    'layer4_connection_flood_check_rule': 'layer4-connection-flood-check-rule',
    'layer3_fragment_protection': 'layer3-fragment-protection',
...
def param_check(module, connection):
    ...
    if res and (action == 'add' or action == 'edit' or action == 'delete') and module.params['name'] is None:
        ...
        res = False
    if res:
        res, err_msg = value_check(module.params, 'enable_http_session_based_prevention', ['enable', 'disable'])
    if res:
        res, err_msg = value_check(module.params, 'enable_layer4_dos_prevention', ['enable', 'disable'])
    if res:
        res, err_msg = value_check(module.params, 'layer3_fragment_protection', ['enable', 'disable'])
...
def main():
    argument_spec = dict(
        ...
        name=dict(type='str'),
        enable_http_session_based_prevention=dict(type='str'),
        http_request_flood_prevention_rule=dict(type='str'),
        http_connection_flood_check_rule=dict(type='str'),
        enable_layer4_dos_prevention=dict(type='str'),
        layer4_access_limit_rule=dict(type='str'),
        layer4_connection_flood_check_rule=dict(type='str'),
        layer3_fragment_protection=dict(type='str'),
